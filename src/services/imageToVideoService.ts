...
export async function generateImageToVideo(
  ...
  try {
    ...
    console.log('Sending Seedance I2V request to API...', requestBody);
    ...
    console.log('Seedance I2V response:', response.data);
    ...
    if (response.data.status === 'success' || response.data.status === 'completed') {
      ...
      if (videoUrl) {
        console.log('Seedance I2V video ready immediately (success status):', videoUrl);
        return {
          videoUrl: videoUrl,
    ...
    if (response.data.proxy_links && response.data.proxy_links.length > 0) {
      const videoUrl = response.data.proxy_links[0];
      console.log('Seedance I2V video from proxy_links:', videoUrl);
      return {
        videoUrl: videoUrl,
        generationTime: response.data.generationTime
      };
    ...
    if (response.data.links && response.data.links.length > 0) {
      ...
      console.log('Seedance I2V video from links:', videoUrl);
      return {
        videoUrl: videoUrl,
        generationTime: response.data.generationTime
      };
    ...
    if (response.data.output && response.data.output.length > 0) {
      ...
      console.log('Seedance I2V video from output:', videoUrl);
      return {
        videoUrl: videoUrl,
        generationTime: response.data.generationTime
      };
    ...
    if (response.data.id && (response.data.status === 'queued' || response.data.status === 'processing' || response.data.status === 'pending')) {
      console.log('Seedance I2V is queued/processing, request ID:', response.data.id);
      return {
    ...
    console.error('Seedance I2V unexpected response:', errorMsg);
    throw new Error(errorMsg);
  } catch (error: any) {
    console.error('Full image-to-video generation error:', {
...
export async function pollForImageToVideo(
  requestId: string,
  maxAttempts: number = 60
): Promise<{ videoUrl: string; generationTime?: number }> {
  console.log('Starting Seedance I2V polling for request:', requestId);
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    try {
      const response = await axios.post<ImageToVideoResponse>(
        `https://modelslab.com/api/v7/video-fusion/fetch/${requestId}`,
        {
          key: API_CONFIG.MODELSLAB_API_KEY
        },
        {
      ...
      console.log(`Seedance I2V poll attempt ${attempt + 1}/${maxAttempts}:`, response.data.status);

      // Check for success with video URLs
      if ((response.data.status === 'success' || response.data.status === 'completed') && 
          (response.data.output || response.data.proxy_links || response.data.links)) {
        const videoUrl = response.data.output?.[0] || response.data.proxy_links?.[0] || response.data.links?.[0];
        if (videoUrl) {
          console.log('Seedance I2V video ready:', videoUrl);
          return {
            videoUrl: videoUrl,
            generationTime: response.data.generationTime
          };
      ...
      if (response.data.eta) {
        console.log(`Seedance I2V ETA: ${response.data.eta}s`);
      }
      
      // Continue polling if still processing
    } catch (error: any) {
      console.error(`Seedance I2V poll error on attempt ${attempt + 1}:`, error.message);
